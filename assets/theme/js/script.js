...
$jscomp.polyfill = function(b, g, d, q) {
	if (g) {
		d = $jscomp.global;
		b = b.split(".");
		...
		q = d[b];
		g = g(q);
		g != q && null != g && $jscomp.defineProperty(d, b, {
			configurable: !0,
			writable: !0,
			value: g
		})
...
$jscomp.polyfill("Array.from", function(b) {
	return b ? b : function(b, d, q) {
		...
		if ("function" == typeof m) {
			b = m.call(b);
			for (var r = 0; !(m = b.next()).done;) g.push(d.call(q, m.value, r++))
		} else
			for (m = b.length, r = 0; r < m; r++) g.push(d.call(q, b[r], r));
		return g
	}
}, "es6", "es3");
$jscomp.owns = function(b, g) {
...
$jscomp.polyfill("Object.values", function(b) {
	return b ? b : function(b) {
		var d = [],
			g;
		for (g in b) $jscomp.owns(b, g) && d.push(b[g]);
		return d
	}
}, "es8", "es3");
...
$jscomp.checkStringArgs = function(b, g, d) {
	...
	if (g instanceof RegExp) throw new TypeError("First argument to String.prototype." + d + " must not be a regular expression");
	return b + ""
};
$jscomp.polyfill("String.prototype.startsWith", function(b) {
	return b ? b : function(b, d) {
		...
		var l = g.length,
			m = b.length;
		d = Math.max(0, Math.min(d | 0, g.length));
		for (var r = 0; r < m && d < l;)
			if (g[d++] != b[r++]) return !1;
		return r >= m
	}
...
(function() {
	...
	function r(a) {
		...
		(function e() {
			var k = parseFloat(a.style.opacity);
			1 < (k += .1) || (a.style.opacity = k, requestAnimationFrame(e))
		})()
	}
	...
	function y(a, c, e) {
		...
		a.setAttribute("data-pie", "{ " + H(a.closest("section")) + ' "percent": ' + e + ', "size": 150, "colorCircle": "#f1f1f1", "stroke": 5, "colorSlice": "url(#' + c + ')", "fontSize": "1.3rem", "number": false }');
	...
	function D() {
		if (document.querySelector(".loop-container")) {
			var a = function(c) {
				var e = 0,
					b = 0,
					h = +c.children[0].dataset.speed || .05,
				...
				if (d) {
					...
					c.children[1].style.cssText = "position: absolute; left: " +
						100 * -f + "%;";
					var g = function() {
							b += h;
							e = .8 * e + .2 * b;
							100 < b && (e -= b, b = 0);
							c.style.transform = "translateX(" + e * f + "%)";
	...
	var f, n, t = "function" == typeof jQuery;
	t && (f = jQuery);
	...
	Element.prototype.parents = function(a) {
		for (var c = [], b = this, k = void 0 !== a; null !== (b = b.parentElement);) b.nodeType === Node.ELEMENT_NODE && (k && !b.matches(a) || c.push(b));
		return c
	};
	Element.prototype.footerReveal = function() {
		function a() {
			!k && c.offsetHeight <= window.outerHeight ? (c.style.zIndex = "-999", c.style.position = "fixed", c.style.bottom = "0", c.style.width = b.offsetWidth + "px", b.style.marginBottom = c.offsetHeight + "px") : (c.style.zIndex = "", c.style.position =
				"", c.style.bottom = "", c.style.width = "", b.style.marginBottom = "")
		}
		var c = this,
	...
	(function(a) {
		var c = function(a, c, b) {
			...
			return function() {
				var h = this,
					d = arguments;
				e ? clearTimeout(e) : b && a.apply(h, d);
		...
		window[a] = function(b) {
			...
			return b ? this.addEventListener("resize", c(b)) :
	...
	l(function() {
			...
			b(a, "[data-bg-video]").forEach(function(a) {
				...
				if (c) {
					var b = new URLSearchParams(c),
						...
						h = c.match(/(http:\/\/|https:\/\/|)?(player.|www.)?(vimeo\.com|youtu(be\.com|\.be|be\.googleapis\.com))\/(shorts\/|video\/|embed\/|watch\?v=|v\/)?([A-Za-z0-9._%-]*)(&\S+)?/),
						d = a.querySelector(".mbr-background-video-preview") || document.createElement("div");
					d.classList.add("mbr-background-video-preview");
					d.style.display = "none";
					d.style.backgroundSize = "cover";
					d.style.backgroundPosition = "center";
					a.querySelector(".mbr-background-video-preview") || a.childNodes[0].before(d);
					var J = function(a, b) {
						var c = window.outerWidth;
						b = window.outerHeight;
						var e = a._opts.width / a._opts.height;
						var h = Math.ceil(c / e);
						h < b && (h = b, c = Math.ceil(h * e));
						a.setSize(c, h)
					};
					if (h && (/youtu\.?be/g.test(h[3]) || /vimeo/g.test(h[3])))
						if (h && /youtu\.?be/g.test(h[3])) {
							h[6] = h[6].replace("shorts", "embed");
							c = "http" + ("https:" === location.protocol ? "s" : "") + ":";
							c += "//img.youtube.com/vi/" + h[6] + "/maxresdefault.jpg";
							var k = new Image;
							k.onload = function() {
								if (120 === (k.naturalWidth || k.width)) {
									var b = k.src.split("/").pop();
									switch (b) {
										case "maxresdefault.jpg":
											k.src = k.src.replace(b, "sddefault.jpg");
											break;
										case "sddefault.jpg":
											k.src = k.src.replace(b, "hqdefault.jpg");
											break;
										default:
											n && (d.style.backgroundImage = 'url("images/no-video.jpg")', d.style.display = "block")
									}
								} else d.style.backgroundImage = 'url("' + k.src + '")', d.style.display = "block";
								...
								var g = k.naturalHeight,
									p = k.naturalWidth,
									F = k.naturalHeight / k.naturalWidth,
						...
						} else h && /vimeo/g.test(h[3]) && (b = new XMLHttpRequest,
							b.open("GET", "https://vimeo.com/api/v2/video/" + h[6] + ".json", !0), b.onreadystatechange = function() {
								if (4 === this.readyState)
									if (200 <= this.status && 400 > this.status) {
										var a = JSON.parse(this.responseText);
										d.style.backgroundImage = 'url("' + a[0].thumbnail_large + '")';
										d.style.display = "block"
									} else n && (d.style.backgroundImage = 'url("images/no-video.jpg")', d.style.display = "block")
							}, b.send(), b = null, a.querySelector(".mbr-background-video") && a.querySelector(".mbr-background-video").remove(), b = document.createElement("div"),
							b.classList.add("mbr-background-video"), a.childNodes[1].before(b), c = new Vimeo.Player(b, {
								id: c,
								loop: !0,
								background: !0,
								responsive: !0,
								...
								muted: !0,
								controls: !1
							}), b = c.element.parentNode, b.style.overflow = "hidden", c.element.style.pointerEvents = "none", c.element.style.marginLeft = "-" + (c.element.scrollWidth - b.scrollWidth) / 2 + "px", c.element.style.minHeight = "100vh", c.element.style.minWidth = "177.77vh");
					else if (n) d.style.backgroundImage = 'url("images/video-placeholder.jpg")', d.style.display =
						"block";
					else if (!n) {
						...
						a.querySelector(".mbr-fallback-image") &&
							(c = window.getComputedStyle(a.querySelector(".mbr-fallback-image")).backgroundImage, c = c.match(/\((.*?)\)/)[1].replace(/('|")/g, ""), b.setAttribute("poster", c));
						b.parentNode.style.overflow = "hidden"
					}
				}
			})
		...
		w() && navigator.userAgent.match(/Chrome/i) ? function(a, b) {
			...
			document.querySelector("html").classList.contains("mbr-site-loaded") &&
				b(a.target, ".mbr-section--full-height").length && window.dispatchEvent(new CustomEvent("resize"))
		}));
		window.addEventListener("smartresize", function() {
			document.querySelectorAll(".mbr-section--16by9").forEach(a)
		});
		if (t) f(document).on("add.cards changeParameter.cards", function(c) {
		...
		if ("undefined" !==
			...
			if (n) {
				...
				f(document).on("changeParameter.cards", function(a, b, e, h) {
					if ("bg" === b) switch (b = a.target, jarallax && jarallax(b, "destroy"), b.style.position = "", f(a.target).find(".mbr-background-video-preview").remove(),
						f(a.target).find(".mbr-background-video").remove(), h) {
						case "type":
		...
		window.addEventListener("scroll", function() {
			...
			if (p) {
				...
				b ? c != p.fixed && (m ? (p.fixed = c, p.elm.classList.toggle("is-fixed")) : h = setTimeout(function() {
					p.fixed = c;
					p.elm.classList.toggle("is-fixed")
				}, 40)) : (p.fixed = !1, p.elm.classList.remove("is-fixed"))
			}
		});
		if (t) f(document).on("add.cards delete.cards", function(a) {
			k && clearTimeout(k);
			k = setTimeout(function() {
					p && (p.fixed = !1, p.elm.classList.remove("is-fixed"));
					var a = document.querySelector(".mbr-fixed-top");
					a && (p = {
						breakPoint: g(a).top + 3 * d(a),
						fixed: !1,
						elm: a
		...
		if (t) f(document).on("add.cards", function(a) {
		...
		if (n) {
			...
			f(document).on("add.cards", function(a) {
				a.target !== document &&
					!r.some(function(b) {
						return b === a.target
					}) && a.target.hasAttribute("mbr-data-bg-video") && (r.push(a.target), e(a.target))
			})
		} else e(document.body);
		if (n) f(document).on("changeParameter.cards", function(a, b, c, h) {
			if ("bg" === b) switch (h) {
				case "type":
		...
		n || document.addEventListener("click",
			function(a) {
				try {
					var b = a.target;
						...
						do
							if (b.hash) {
								...
								document.querySelectorAll(c ? "body" : b.hash).forEach(function(c) {
									...
									e = "#bottom" == b.hash ? d(c) - window.innerHeight : g(c).top - (e ? 60 : 0);
		...
		document.querySelectorAll(".cols-same-height .mbr-figure").forEach(function(a) {
			function b() {
				...
				if (k && h) {
					...
					a.style.bottom = "0";
					var f = d(e) / q(e);
					f > b && (b = 100 * (f - b) / b, c.style.width = b + 100 + "%", c.style.maxWidth = b + 100 + "%", c.style.marginLeft = -b / 2 + "%")
				}
			}
	...
	if (!n) {
		if (t && f.fn.socialLikes) f(document).on("add.cards", function(a) {
		...
		l(function() {
			if (!w() && document.querySelectorAll("input[name=animation]").length) {
				var a = function() {
						...
						d.forEach(function(h) {
							var d = h.offsetHeight,
								k = e(h);
							(k + d >= a && k - 50 <= c || b(h)) && h.classList.contains("hidden") && (h.classList.remove("hidden"), h.classList.add("animate__fadeIn"), h.classList.add("animate__delay-1s"), h.addEventListener("webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend",
								function() {
									h.classList.remove("animate__animated animate__delay-1s animate__fadeIn")
								}, {
									once: !0
								}))
						})
				...
				var d = Array.from(document.querySelectorAll("p, h1, h2, h3, h4, h5, a, button, small, img, li, blockquote, .mbr-author-name, em, label, input, select, textarea, .input-group, .form-control, .iconbox, .btn-social, .mbr-figure, .mbr-map, .mbr-testimonial .card-block, .mbr-price-value, .mbr-price-figure, .dataTable, .dataTables_info, .item"));
				d = d.filter(function(a) {
					if (!a.parents().filter(function(a) {
							if (a.matches("a, p, .navbar, .mbr-arrow, footer, .iconbox, .mbr-slider, .mbr-gallery, .mbr-testimonial .card-block, #cookiesdirective, .mbr-wowslider, .accordion, .tab-content, .engine, #scrollToTop, .modal, .item")) return !0
						}).length) return !0
				});
				d = d.filter(function(a) {
		...
		if (document.querySelectorAll(".mbr-arrow-up").length) {
			...
			window.addEventListener("scroll", function() {
				(document.documentElement.scrollTop || document.body.scrollTop) > Math.max(document.body.scrollHeight, document.documentElement.scrollHeight, document.body.offsetHeight,
					document.documentElement.offsetHeight, document.body.clientHeight, document.documentElement.clientHeight) / 2 - document.documentElement.clientHeight / 2 || window.scrollY > window.innerHeight ? r(a) : m(a)
			});
	...
	if (!n) {
		...
		u && u.addEventListener("click", function(a) {
			...
			window.scrollTo(0, g(a).top)
		})
	}
	document.querySelectorAll("nav.navbar").length && (u = d(document.querySelector("nav.navbar")), document.querySelector(".mbr-after-navbar.mbr-fullscreen") && (document.querySelector(".mbr-after-navbar.mbr-fullscreen").style.paddingTop = u + "px"));
	if (!n && (0 < window.navigator.userAgent.indexOf("MSIE ") || navigator.userAgent.match(/Trident.*rv:11\./))) f(document).on("add.cards", function(a) {
		var b = a.target;
			...
			var a = function(a) {
				...
				b = function(b) {
					...
					var g = b.parents("section")[0],
						l = g.querySelector(g.querySelector("[data-modal]").getAttribute("data-modal"));
					l.style.display = "table";
					l.addEventListener("click", function(b) {
						"VIDEO" !== b.target.tagName && (e ? (-1 !== e.indexOf("youtu") && c.contentWindow.postMessage('{"event":"command","func":"pauseVideo","args":""}', "*"), -1 !== e.indexOf("vimeo") && d.pause()) : (b = c.parentNode.querySelector("video")) && b.pause && b.pause(), l.style.display = "none", l.removeEventListener("click", a), g.style.zIndex = "0")
					})
				};
	...
	if (!n) {
		...
		!w() && (u = document.querySelector("section.menu")) && (z = window.innerWidth, !u.querySelector(".navbar").classList.contains("collapsed") && 991 < z && (u.querySelectorAll("ul.navbar-nav li.dropdown").forEach(function(a) {
	...
	if (n) {
		...
		f(document).on("add.cards", function(a) {
			f(a.target).find(".form-with-styler").length &&
				(a = f(a.target).find(".form-with-styler"), f(a).find('select:not("[multiple]")').each(function() {
					f(this).styler && f(this).styler()
				}), f(a).find("input[type=number]").each(function() {
					f(this).styler && (f(this).styler(), f(this).parent().parent().removeClass("form-control"))
	...
	if (n) f(document).on("add.cards changeParameter.cards", function(a, b) {
		"undefined" !== typeof CircularProgressBar && new CircularProgressBar("pie_progress");
		b ? I(a.target, b) : a.target.querySelectorAll(".pie_progress").length && a.target.querySelectorAll(".pie_progress").forEach(function(a) {
			C(a)
		})
	});
	...
	if (n && t) f(document).on("add.cards", function(a) {
		f(a.target).hasClass("testimonials-slider") && x(a.target)
	}).on("changeParameter.cards", function(a, b, d) {
		"testimonialsSlides" === b && 0 == f(a.target).find(".carousel-item.active").length && A(a.target)
	});
	...
	l(function() {
		var a = !0,
			b = function() {
				...
				if (b &&
					!b.classList.contains("opacityScrollOff")) {
					var c = 1 < document.documentElement.scrollTop;
					if (c !== a || b.classList.contains("opacityScroll")) {
						var d = document.querySelector(".navbar-collapse");
						a = c;
						b.classList.toggle("opacityScroll", !c);
						d && d.classList.toggle("opacityScroll", !c)
					}
				}
			};
		b();
